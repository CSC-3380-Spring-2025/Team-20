...
interface EventCardProps {
  eventData: EventData;
  onCardClick?: () => void;
  onSave: (event: Omit<EventData, 'id'>) => Promise<void> | void;
  onDelete: () => void;
  isSubmitting?: boolean;
}

const EventCard = ({ eventData, onCardClick = () => {}, onSave, onDelete }: EventCardProps) => {
      ...
      <div className="p-4">
        ...
        {eventData.totalInterested > 0 && (
          <p className="text-sm text-green-700 mt-2 font-medium">
            🎉 {eventData.totalInterested}{" "}
            {eventData.totalInterested === 1 ? "person is" : "people are"} going{" "}
          ...
          >
            ...
            {!showEventForm ? (
              ...
              <div className="mt-6 bg-gray-100 p-4 rounded-xl shadow-inner max-h-[70vh] overflow-y-auto">
                <EventForm
                  onSave={(data) => {
                    const adaptedData: Omit<EventData, 'id'> = {
                      ...data,
                      link: "", 
                      image: "", 
                      name: "", 
